...
if(!empty($photo)){
		...
		?>
        <div class="cover" style="background-image:url('IMG/<?PHP ECHO $photo ?>'),linear-gradient(45deg, #cddc3942, #0dbee67a);  
...
<div class="titleb<?php echo $column->ID?>"><a href="index.php?box_ID=<?php echo $column->ID?>"  id='linker' ><?php echo  $column->title ?></a>
...
$select_topic="SELECT *  from topic where  content_id  = $MAIN_ID";
#$query=mysqli_query($connect,$SELECT);

$topicquery=$this->connectF()->query($select_topic);

while($column=mysqli_fetch_object($topicquery)){
