...
<body>
<form  action='Editboxboxes.php' method='post'>
...
class Editboxboxes  extends  condb{        


public function sql(){

@$MAIN_ID=$_POST['box_ID'];
$select="select * from  box  where  related_id = $MAIN_ID ";
...
<td><form action="cat_boxboxes.php" method="post">
<INPUT TYPE='hidden' name='MainM_ID'  value='<?php echo $data->related_id?>'/>
<INPUT TYPE='hidden' name='cat_type'  value='<?php echo $data->cat_type?>'/>
...
@$_SET=$_POST['checkbox'];	
    ...
    $del_cat="delete from box  where  related_id = '$Box_id' And  cat_type='$cat_type'";
...
$upbox="update box SET    cat_Name='$full_cat'  ,  cat_type='$cat_type'      where  ID='$ID' ";
