...
<body>
<form  action='Edittopicbox.php' method='post'>
...
$MAIN_ID=$_POST['Topic_ID'];
$select="select * from  box  where  relatedtopic_id = $MAIN_ID ";
...
<td><form action="cat_boxtopic.php" method="post">
<INPUT TYPE='hidden' name='MainM_ID'  value='<?php echo $data['relatedtopic_id'];?>'/>
<INPUT TYPE='hidden' name='cat_type'  value='<?php echo $data['cat_type'];?>'/>
  ...
  if(@$_POST['delete']){	
	...
	@$DEL_ID=implode(",",$_SET);		
	  ...
	  #$DELETE="delete from slider where ID = $_SET ";
          $del_cat="delete from box  where  relatedtopic_id = '$Box_id' And  cat_type='$cat_type'";
